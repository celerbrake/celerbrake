...
require 'curb' unless Celerbrake::JRUBY
require 'excon'
...
class DummyApp < Rails::Application
  ...
  # https://app.circleci.com/pipelines/github/celerbrake/celerbrake/jobs/7996
  #
  ...
  routes.append do
    ...
    get '/notify_celerbrake_helper' => 'dummy#notify_celerbrake_helper'
    get '/notify_celerbrake_sync_helper' => 'dummy#notify_celerbrake_sync_helper'
...
class Book < ActiveRecord::Base
  ...
  def raise_error_after_commit
    raise CelerbrakeTestError, 'after_commit'
  end

  def raise_error_after_rollback
    raise CelerbrakeTestError, 'after_rollback'
  end
end
...
class BingoJob < ActiveJob::Base
  ...
  def perform(*_args)
    ...
    raise CelerbrakeTestError, 'active_job error'
  end
end
...
class BingoWorker
  ...
  def self.perform(_bango, _bongo)
    raise CelerbrakeTestError, 'resque error'
  end
end
...
BangoJob = Struct.new(:bingo, :bongo) do
  def perform
    raise CelerbrakeTestError, 'delayed_job error'
  end
end
...
class DummyController < ActionController::Base
  ...
  self.view_paths = [
    ActionView::FixtureResolver.new(
      ...
      'dummy/notify_celerbrake_helper.html.erb' => 'notify_celerbrake_helper',
      'dummy/notify_celerbrake_sync_helper.html.erb' => 'notify_celerbrake_helper_sync',
  ...
  def notify_celerbrake_helper
    notify_celerbrake(CelerbrakeTestError.new)
  end

  def notify_celerbrake_sync_helper
    notify_celerbrake_sync(CelerbrakeTestError.new)
  end
