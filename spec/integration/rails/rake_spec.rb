...
RSpec.describe "Rake integration" do
  ...
  before do
    ...
    allow(Celerbrake).to receive(:notify_sync)
  end

  after do
    expect { task.invoke }.to raise_error(CelerbrakeTestError)
  ...
  it "sends the exception to Celerbrake" do
    expect(Celerbrake).to receive(:notify_sync)
      .with(an_instance_of(Celerbrake::Notice))
  end
  ...
  describe "contains the params payload, which" do
    ...
    it "includes a timestamp" do
      ...
      expect(Celerbrake).to receive(:notify_sync).with(expected_notice)
    end
    ...
    it "includes locations" do
      expect(Celerbrake).to receive(:notify_sync) do |notice|
