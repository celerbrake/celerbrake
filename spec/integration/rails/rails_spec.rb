...
RSpec.describe "Rails integration specs" do
  ...
  let(:config) { Celerbrake::Config.instance }

  let!(:endpoint) do
    stub_request(:post, 'https://api.celerbrake.com/api/v3/projects/113743/notices')
  ...
  before do
    stub_request(:put, %r{https://api.celerbrake.com/api/v5/projects/113743/.+})
      .to_return(status: 201, body: '{}')

    allow(Celerbrake).to receive(:notify_request)
    allow(Celerbrake).to receive(:notify_query)
    allow(Celerbrake).to receive(:notify_performance_breakdown)
  ...
  if ::Rails.version.to_i >= 5
    it "inserts the Celerbrake Rack middleware after DebugExceptions" do
      middlewares = Rails.configuration.middleware.middlewares.map(&:inspect)
      own_idx = middlewares.index('Celerbrake::Rack::Middleware')

      expect(middlewares[own_idx - 1]).to eq('ActionDispatch::DebugExceptions')
    end
  else
    it "inserts the Celerbrake Rack middleware after ConnectionManagement" do
      middlewares = Rails.configuration.middleware.middlewares.map(&:inspect)
      own_idx = middlewares.index('Celerbrake::Rack::Middleware')
  ...
  shared_examples "context payload content" do |route|
    ...
    it "includes action information" do
      body =
        ...
        when '/notify_celerbrake_helper'
          /"context":{.*"action":"notify_celerbrake_helper".*}/
        when '/notify_celerbrake_sync_helper'
          /"context":{.*"action":"notify_celerbrake_sync_helper".*}/
        else
          raise 'Unknown route'
        end
      expect(a_request(:post, endpoint).with(body: body)).to have_been_made
  ...
  describe "context payload" do
    ...
    context "when exception reported through the notify_celerbrake helper" do
      include_examples('context payload content', '/notify_celerbrake_helper')
    end

    context "when exception reported through the notify_celerbrake_sync helper" do
      include_examples('context payload content', '/notify_celerbrake_sync_helper')
    end
  end
  ...
  describe(
    ...
    it "reports exceptions in after_commit callbacks" do
      expect(Celerbrake).to receive(:notify).with(
        an_instance_of(CelerbrakeTestError),
      ) do |exception|
  ...
  if Gem::Version.new(Rails.version) >= Gem::Version.new('4.2')
    describe "ActiveJob jobs" do
      it "reports exceptions occurring in ActiveJob workers" do
        expect(Celerbrake).to receive(:notify)
          .with(an_instance_of(Celerbrake::Notice)).at_least(:once)
      ...
      context "when Celerbrake is not configured" do
        before do
          ...
          # that: https://github.com/celerbrake/celerbrake/pull/583
          allow_any_instance_of(Celerbrake::Rack::Middleware).to(
            receive(:notify_celerbrake),
          )
        end

        it "doesn't report errors" do
          expect(Celerbrake).to receive(:notify).with(
            an_instance_of(Celerbrake::Notice),
          ) { |notice|
  ...
  describe "Resque workers" do
    it "reports exceptions occurring in Resque workers" do
      expect(Celerbrake).to receive(:notify_sync).with(
        anything,
        hash_including(
          'class' => 'BingoWorker',
          'args' => %w[bango bongo],
  ...
  describe "request performance hook" do
    ...
    it "notifies request" do
      expect(Celerbrake).to receive(:notify_request).with(
        hash_including(
          route: '/crash(.:format)',
          method: 'GET',
        ),
    ...
    context "when Rails version is below 7" do
      ...
      it "reports time as an instance of Time" do
        head '/crash'
        expect(Celerbrake).to have_received(:notify_request).with(
  ...
  describe "query performance hook" do
    ...
    it "sends queries to Celerbrake" do
    ...
    context "when caller location cannot be found for a query" do
      ...
      it "sends query to Celerbrake without caller location" do
        expect(Celerbrake).to receive(:notify_query).with(
          hash_including(
            route: '/crash(.:format)',
            method: 'GET',
  ...
  describe "performance breakdown hook" do
    ...
    it "sends performance breakdown info to Celerbrake" do
    ...
    context "when response format is */*" do
      it "normalizes it to :html" do
        expect(Celerbrake).to receive(:notify_performance_breakdown)
    ...
    context "when an action performs a Curl request" do
      ...
      before { skip("JRuby doesn't support Curb") if Celerbrake::JRUBY }
    ...
    context "when an action performs a Typhoeus request" do
      ...
      before do
        ...
        skip('SIGSEGV on JRuby 9.2.19.0') if Celerbrake::JRUBY
      end
