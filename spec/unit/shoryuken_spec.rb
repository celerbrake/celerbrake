...
require 'celerbrake/shoryuken'

RSpec.describe Celerbrake::Shoryuken::ErrorHandler do
  let(:error) { CelerbrakeTestError.new('shoryuken error') }
  ...
  context "when the worker finishes without an error" do
    before { allow(Celerbrake).to receive(:notify) }
