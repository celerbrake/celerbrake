...
RSpec.describe Celerbrake::Rack::UserFilter do
  def env_for(url, opts = {})
    Rack::MockRequest.env_for(url, opts)
  end

  let(:notice) do
    Celerbrake.build_notice('oops').tap do |notice|
      notice.stash[:rack_request] = Rack::Request.new(env_for('/', {}))
    end
  end

  let(:user_payload) { { username: 'bingo' } }
  let(:user) { Celerbrake::Rack::User.new(double(user_payload)) }
  ...
  context "when no current user is found" do
    let(:user) { Celerbrake::Rack::User.new(double) }
